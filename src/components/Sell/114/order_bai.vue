<template>
  <div class="order-confirm">
    ...
    <div class="center-box">
      <div class="service-name">欢乐保服务订单</div>
      <div class="center_title" :class="{font_black: payState}">硬件信息</div>
      ...
      <div class="confirm-pay-btn">
        <mt-button type="primary" size="large" @click="structurePayUrl()">去支付
        </mt-button>
      </div>
...
<script>
  ...
  export default {
    ...
    async created () {
      ...
      await loadData(() => { return api.getOrderConfirm(this.orderId) },
        (responseOrder) => {
          this.orderCode = responseOrder.content.orderCode
    ...
    methods: {
      ...
      async structurePayUrl () {
        await loadData(() => {
          return api.postBaiPayConfirm(this.paymentId, this.param)
        }, (res) => {
          if (res) {
            window.location.href = 'http://service.114sl.cn/wx.php?m=Zan&a=getPayParam&orderId=' + this.orderCode
          }
        }, boolPreHandler, true)
      }
    }
