  ...
  <div class="Brand bg_img_none">
    ...
    <div class="list">
      ...
      <div class="content" v-infinite-scroll="loadMore"
           infinite-scroll-disabled="brandList.loading" infinite-scroll-distance="300">
...
export default {
  ...
  methods: {
    ...
    async getPopularBrandList () {
      ...
      await loadData(() => {
        return api.getPopularBrand(popularBrand)
      }, data => {
        if (data.isOk) {
          this.popularBrand = data.content
        }
      }, null)
    },
    async getBrandList () {
      let params = {
        ...
        categoryId: this.categoryId,
        name: this.filter,
        limit: this.brandList.limit,
        page: this.brandList.page
      }
      if (this.brandList.page === 1) {
        this.brandList.list = []
        this.dataSource = []
      }
      await loadData(() => { return api.getBrands(params) }, (brandList) => {
        this.setIndex(brandList)
        this.dataSource.push(...brandList)
      }, defaultPreHandler, true)
      this.brandList.noMore = this.dataSource.length < (this.brandList.page * this.brandList.limit)
    },
    setIndex (data) {
      _.forEach(data, (item) => {
