...
import getPrefix from './modules/get_prefix'
import { requestAnimationFrame, cancelAnimationFrame } from './modules/request_animation_frame'
...
class LotteryDial extends Events {
  constructor (pointer, options) {
    ...
    this.options = Object.assign({
      speed: 30, // 每帧速度
      // areaNumber: 7, // 奖区数量
      deviation: 5 // 随机结果角度偏差值 为了防止出现指针和扇区分割线无限重合 单位:°
    }, options)
  ...
  setResult (index) {
    ...
    endAngle = Math.max(this.options.deviation, endAngle)  // 返回两个指定的数中带有较大的值的那个数。
    endAngle = Math.min(singleAngle - this.options.deviation, endAngle)  // 单个奖区角度值 - 随机结果角度偏差值 为了防止出现指针和扇区分割线无限重合 单位:°
    endAngle = Math.ceil(endAngle + (index * singleAngle))  // 向上取整
